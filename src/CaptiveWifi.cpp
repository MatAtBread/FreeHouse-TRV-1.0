...
cl.println("</table><button onclick='window.location.href = \"/close\"'>Close</button></body></html>");
}
...
CaptivePortal::CaptivePortal(Heartbeat* heartbeat, Trv* trv) : heartbeat(heartbeat), server(80), trv(trv) {
  ...
  Serial.printf(F("Started Captive Portal %s\n"), accessPointName);
}
        ...
        if (client.available()) {  // if there's bytes to read from the client,
          ...
          if (c == '\n') {  // if the byte is a newline character
            ...
            if (currentLine.length() == 0) {
              ...
              if (header.startsWith("GET /close")) {
                close = true;
              }
