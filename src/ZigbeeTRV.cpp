...
void ZigbeeTRV::zbAttributeRead(uint16_t cluster_id, const esp_zb_zcl_attribute_t *attribute) {
  Serial.printf("Attribute read: cluster %04X, attr %04X, value %d\n", cluster_id, attribute->id, *(uint8_t *)attribute->data.value);
    ...
    switch (message->attribute.id) {
      ...
      case ESP_ZB_ZCL_ATTR_THERMOSTAT_OCCUPIED_HEATING_SETPOINT_ID:
        if (message->attribute.data.type == ESP_ZB_ZCL_ATTR_TYPE_S16) {
          ...
          Serial.printf("Set heating setpoint: %f\n", zb_s16_to_temperature(value));
          return;
        }
        break;
      case ESP_ZB_ZCL_ATTR_THERMOSTAT_SYSTEM_MODE_ID:
        if (message->attribute.data.type == ESP_ZB_ZCL_ATTR_TYPE_8BIT_ENUM) {
          Serial.printf("Set system mode: %s\n", systemModeToString((esp_zb_zcl_thermostat_system_mode_t)*(uint8_t *)message->attribute.data.value));
          return;
        }
        break;
      case ESP_ZB_ZCL_ATTR_THERMOSTAT_LOCAL_TEMPERATURE_CALIBRATION_ID:
        if (message->attribute.data.type == ESP_ZB_ZCL_ATTR_TYPE_S8) {
          ...
          Serial.printf("Set temp calibration: %f\n", value/10.0);
          return;
        }
        break;
      case MANU_THERMOSTATE_VALUE_POSITION_ID:
        if (message->attribute.data.type == ESP_ZB_ZCL_ATTR_TYPE_U8) {
          ...
          Serial.printf("Set valve position: %d&&\n", value);
          return;
        }
        break;
    }
    Serial.printf("Unknown attribute set: cluster %04X, attr %04X, value %p\n", message->info.cluster, message->attribute.id, message->attribute.data.value);
    return;
  }
}
