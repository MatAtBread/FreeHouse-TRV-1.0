...
const trv_state_t defaultState = {
    ...
    .config = {.current_heating_setpoint = 21.5,
               ...
               .resolution = 1,
               .debug_flags = 0,
               .motor = {.reversed = true, .backoff_ms = 259, .stall_ms = 1}}};

uint32_t debugFlag(DebugFlags mask) {
  return globalState.config.debug_flags & (uint32_t)mask;
}

const char *Trv::deviceName() {
  return globalState.config.mqttConfig.device_name;
}
...
// On construction, the state is guaranteed to be valid, and asynchronously the
// sensors, etc are initialized
Trv::Trv() {
  mustCalibrate = false;
  configDirty = false;
  fs = new TrvFS();
  ...
  if (esp_reset_reason() == ESP_RST_DEEPSLEEP &&
    ...
    if (r && (r <= sizeof(state) || state.version < STATE_VERSION)) {
      UPDATE_STATE(7, state.config.debug_flags = 0;
                   state.config.motor.backoff_ms = BACKOFF_MS_DEFAULT;
                   state.config.motor.stall_ms = STALL_MS_DEFAULT;)
      r = sizeof(state);
    }

    if (r != sizeof(state) || state.version != STATE_VERSION) {
      ESP_LOGI(TAG, "Default state, version %lu != %lu", state.version,
               STATE_VERSION);
  ...
  // We crerate the battery monitor here, as it's fast and not task based, which
  // makes testing for a flat battery quick
...
void Trv::task() {
  ...
  tempSensor = new DallasOneWire(globalState.sensors.sensor_temperature,
                                 globalState.config.resolution);
  motor = new MotorController(battery, globalState.sensors.position,
                              globalState.config.motor);
  if (mustCalibrate) {
    motor->calibrate();
  }
  if (!mcuTempSensor)
    mcuTempSensor = new McuTempSensor(); // Lazily get MCU temp

  globalState.sensors.is_charging = battery->is_charging();
  globalState.sensors.position = motor->getValvePosition();
  // We only update battery values when the motor is off. If it's moving, it
  // will drop due to the loading
  if (motor->getDirection() == 0) {
    globalState.sensors.battery_raw = battery->getValue();
    globalState.sensors.battery_percent =
        battery->getPercent(globalState.sensors.battery_raw);
  }

  // We choose to keep a running average the temp to minimize the heating effect
  // of operating We don't read the temperature if we are charging, as the MCU
  // is hot
  float compensation = 0.0;
  if (globalState.sensors.is_charging) {
    compensation = 1.0; // TODO: calibrate this value from the MCU temperature
  }
  auto local = tempSensor->readTemp() +
               globalState.config.local_temperature_calibration + compensation;
  globalState.sensors.local_temperature =
      (local + globalState.sensors.local_temperature) / 2;
}

bool Trv::requiresNetworkControl() { return otaUrl.length() > 0; }

Trv::~Trv() {
  wait();
  if (configDirty)
    saveState(); // Update NVS if necessary
  if (this->otaUrl.length()) {
    doUpdate();
  }
  if (mcuTempSensor)
    delete mcuTempSensor;
  delete motor;
  delete tempSensor;
  delete battery;
  delete fs;
...
std::string Trv::asJson(const trv_state_t &s, signed int rssi) {
  ...
  json << "\"mcu_temperature\":" << mcuTemp
       << ","
          "\"local_temperature\":"
       << s.sensors.local_temperature
       << ","
          "\"sensor_temperature\":"
       << s.sensors.sensor_temperature
       << ","
          "\"battery_percent\":"
       << (int)s.sensors.battery_percent
       << ","
          "\"battery_mv\":"
       << (int)s.sensors.battery_raw
       << ","
          "\"is_charging\":"
       << (s.sensors.is_charging ? "true" : "false")
       << ","
          "\"position\":"
       << (int)s.sensors.position
       << ","
          "\"motor\":\""
       << MotorController::lastStatus
       << "\","
          "\"current_heating_setpoint\":"
       << s.config.current_heating_setpoint
       << ","
          "\"local_temperature_calibration\":"
       << s.config.local_temperature_calibration
       << ","
          "\"system_mode\":\""
       << systemModes[s.config.system_mode]
       << "\","
          "\"sleep_time\":"
       << s.config.sleep_time
       << ","
          "\"resolution\":"
       << (0.5 / (float)(1 << s.config.resolution))
       << ","
          "\"backoff_ms\":"
       << s.config.motor.backoff_ms
       << ","
          "\"stall_ms\":"
       << s.config.motor.stall_ms
       << ","
          "\"motor_reversed\":"
       << (s.config.motor.reversed ? "true" : "false")
       << ","
          "\"debug_flags\":"
       << s.config.debug_flags
       << ","
          "\"unpair\":false,"
          "\"calibrate\":false"
          "}";
...
void Trv::doUnpair() { EspNet::unpair(); }

void Trv::saveState() {
  globalState.sensors.position =
      motor->getValvePosition(); // Should be benign as MotorController is
                                 // passed a reference to this value
...
void Trv::setMotorParameters(const motor_params_t &params) {
  if (params.reversed == true || params.reversed == false) {
    if (globalState.config.motor.reversed != params.reversed) {
      ...
      motor->setValvePosition(pos ? 100 : 0); // Re-apply current position to
...
const trv_config_t &Trv::getConfig() { return globalState.config; }
...
void Trv::setNetMode(net_mode_t mode, trv_mqtt_t *mqtt) {
  ...
  if (changed) {
    configDirty = true;
    // We should probably do a restart to make sure there are no clashes with
    // other operations
  }
}

void Trv::setPassKey(const uint8_t *key) {
  if (memcmp(globalState.config.passKey, key,
             sizeof(globalState.config.passKey)) != 0) {
...
void Trv::setTempCalibration(float temp) {
  if (globalState.config.local_temperature_calibration == temp)
    return;
...
void Trv::setHeatingSetpoint(float temp) {
  if (globalState.config.current_heating_setpoint == temp)
    return;
...
void Trv::checkAutoState() {
  ...
  if (state.config.system_mode == ESP_ZB_ZCL_THERMOSTAT_SYSTEM_MODE_AUTO) {
    if (state.sensors.local_temperature >
        state.config.current_heating_setpoint) {
      motor->setValvePosition(0);
    } else if (state.sensors.local_temperature <
               state.config.current_heating_setpoint - 0.5) {
      motor->setValvePosition(100);
    }
  }
}
...
void Trv::testMode(TouchButton &touchButton) {
  ...
  while (true) {
    ...
    if (motor->getValvePosition() != target) {
      ESP_LOGW(TAG, "Failed to reach target %d, got %d. Sleeping.", target,
               motor->getValvePosition());
    }
    delay(5000);
    touchButton.reset();
